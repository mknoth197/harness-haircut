...
- **U2.** The package shall expose a single CLI binary `harness-haircut` whose entry script lives at `dist/cli.js`.
...
- [ ] `package.json` declares `"type": "module"`, `"bin": { "harness-haircut": "dist/cli.js" }`, and `engines.node >= 20`.
- [ ] `tsconfig.json` configured for ESM, strict mode, `outDir: dist`.
- [ ] `npm run build` produces `dist/cli.js` with a shebang.
- [ ] `npm test` runs and passes a smoke test that spawns the built binary and asserts on `--version` output.
- [ ] `npm run lint` runs (ESLint) and passes on the scaffold.
- [ ] `npx harness-haircut audit` (post-build) prints `not yet implemented` and exits 70.
