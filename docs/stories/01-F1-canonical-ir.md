...
- **EV4.** When the parser encounters a `.agents/hooks/<event>.<name>.<ext>` file, it shall produce one `Hook` with that event, name, and the file's body as `script`.
- **EV5.** When the parser encounters an unrecognized file under `.agents/`, it shall include it in the IR as an opaque `attachment` and emit a `WARN HH-W010 unknown attachment`.
...
- **UN4.** If a hook filename's `<event>` segment is not in the canonical `HookEvent` enum, then the parser shall fail with exit code 3 naming the file and listing valid events.
...
- [ ] Parser at `src/gateways/` + `src/use-cases/` boundary with signature `parseRepo(cwd: string): Promise<IR>` (filesystem access in the gateway; assembly logic pure).
