...
- **EV1.** When the combined size of root + nested `AGENTS.md` content exceeds 32 KiB (Codex's default `project_doc_max_bytes`), the adapter shall emit warning `HH-W004` (provider size cap exceeded — content past the cap is silently dropped by Codex).
