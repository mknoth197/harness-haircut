...
- **EV2.** When the user config sets `gemini.mode: "shim"` (in `harness-haircut.config.json`, PRD §8), the adapter shall instead emit a `GEMINI.md` whose first line is the import `@AGENTS.md` (Gemini import syntax, maxDepth 5) — no SignedSource header, per the §9 shim carve-out.
