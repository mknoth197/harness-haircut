...
- **EV2b.** When the IR contains a nested `AGENTS.md` instruction, the adapter shall emit `.github/instructions/hh.nested-<dirpath>.instructions.md` with `applyTo: "<dir>/**"` so the **code-review surface** (which reads neither root nor nested AGENTS.md) still receives the content — otherwise nested instructions the tool already lifted into the IR would be silently lost on review, violating Goal 3 / §16 "no silent loss of canonical sources the tool reaches" (a fully-gitignored directory is the documented exception per HH-W012's "Scope / known limit"). Other Copilot surfaces read nested AGENTS.md natively and ignore the duplication cleanly per the precedence rules.
...
- **OPT2.** Where a canonical hook event has no Copilot equivalent, the adapter shall emit warning `HH-W003` and skip it. The mapping table includes `pre-tool-use` → `preToolUse` (note: **fail-closed** on the cloud agent), `user-prompt-submit` → `userPromptSubmitted`, `stop` → `agentStop`, `session-start`/`session-end` → `sessionStart`/`sessionEnd`. The fail-closed and default-branch operational notes cannot live in the emitted hooks file itself (JSON carries no comments), so the adapter exports them as `COPILOT_HOOK_NOTES` for the C-series commands to surface informationally.
