...
- **OPT1 (regulated default-off).** Where `harness-haircut.config.json` sets `init.assist.endpointPolicy: "approved-only"` (recommended default for corporate installs), `--assist` shall refuse any provider/endpoint not on the configured allowlist. (#46) When the policy filters out **every** discovered source, the message shall name the POLICY (count of sources discovered + `init.assist.endpointPolicy` + the allowlist state) and the remedy (add to `init.assist.approved` / relax the policy / drop `--assist`) — distinct from the genuine no-credentials message, so a user is not sent chasing CLI logins for an admin-locked-down config.
